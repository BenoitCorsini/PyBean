...
BS.new_brush('Circle', xy=(-1, 1), radius=0.8)
...
BS.new_path_from_raw(vertices, key='tri')
BS.set('tri', lw=50, joinstyle='round', capstyle='round', color='gold')
BS.grid(left=0, bottom=0, steps=(0.4, 0.5), lw=10, **grid_params)

# in the bottom-left corner, plots a square and a grid
side = 0.8*2**0.5
BS.new_brush('Rectangle', 'diam', xy=(0, 0), width=side, height=side)
BS.apply('set_angle', 'diam', angle=45)
BS.apply('set_xy', 'diam', xy=(-1, -1.8))
BS.grid(right=0, top=0, blocks=3, lw=30, **grid_params)

# in the bottom-right corner, plots some text and a grid
path = BS.string_to_path(s='B', xy=(1, -1), height=1.25)
BS.new_path(path)
